...
export class RegistrarInput extends Component {
  ...
  execute() {
    ...
    try {
      ...
      if (
        "update" === this.state.operation ||
        "deactivate" === this.state.operation
      ) {
    ...
    axios
      .post(
        env.backendUrl +
          ...
          "?" +
          "method=" +
          encodeURIComponent(this.state.method),
        JSON.stringify(data)
      )
      .then((response) => {
      ...
      .catch((error) => {
        if (error.response && error.response.data) {
          ...
          this.props.onError(
            String(error) + ": " + JSON.stringify(error.request)
          );
  ...
  render() {
      ...
      executeClearButtons = (
        <Grid columns="16" divided>
          <Grid.Row>
            <Grid.Column width="4">
              <Item>
                <Button primary onClick={this.onClickExecute.bind(this)}>
                  {this.state.operation} → {this.state.driverName}
                </Button>
                <Button secondary onClick={this.onClickClear.bind(this)}>
                  Clear
                </Button>
              </Item>
            </Grid.Column>
    ...
    if (!this.state.operation) {
      const createButtonsList = this.props.drivers.map((driver, i) => (
        <Button
          ...
          primary
          key={i}
          onClick={this.onClickCreate.bind(this, driver.method, driver.name)}
        >
          {driver.name}
        </Button>
      ));
      createButtons = (
        <Item className="buttons">
          <Label>
            <label htmlFor={"createButtonsList"}>Create:</label>
          </Label>
          <span id={"createButtonsList"}>{createButtonsList}</span>
        </Item>
      );
      const updateButtonsList = this.props.drivers.map((driver, i) => (
        <Button
          ...
          primary
          key={i}
          onClick={this.onClickUpdate.bind(this, driver.method, driver.name)}
        >
          {driver.name}
        </Button>
      ));
      updateButtons = (
        <Item className="buttons">
          <Label>
            <label htmlFor={"updateButtonsList"}>Update:</label>
          </Label>
          <span id={"updateButtonsList"}>{updateButtonsList}</span>
        </Item>
      );
      const deactivateButtonsList = this.props.drivers.map((driver, i) => (
        <Button
          ...
          primary
          key={i}
          onClick={this.onClickDeactivate.bind(
            this,
            driver.method,
            driver.name
          )}
      ...
      deactivateButtons = (
        <Item className="buttons">
          <Label>
            <label htmlFor={"deactivateButtonsList"}>Deactivate:</label>
          </Label>
          <span id={"deactivateButtonsList"}>{deactivateButtonsList}</span>
        </Item>
      );
    }
    ...
    if (this.state.options !== null) {
      optionsInput = (
        <Table.Cell>
          <Item>
            <Item className="options-label" for={"optionsInput"}>
              OPTIONS:
            </Item>
            <TextArea
              id={"optionsInput"}
    ...
    if (this.state.secret != null) {
      secretInput = (
        <Table.Cell>
          <Item>
            <Item className="secret-label" for={"secretInput"}>
              SECRET:
            </Item>
            <TextArea
              id={"secretInput"}
    ...
    if (this.state.didDocument != null) {
      didDocumentInput = (
        <Item>
          <Item className="diddocument-label" for={"didDocumentInput"}>
            DID DOCUMENT:
          </Item>
          <TextArea
            id={"didDocumentInput"}
            className="diddocument"
            value={this.state.didDocument}
            cols="60"
            rows="5"
    ...
    if (
      ...
      if (Object.keys(addServices).length > 0) {
        addServicesList = <List>{addServices}</List>;
      }
  ...
  defaultOptions(operation, method) {
    ...
    return options[operation][method]
      ? JSON.stringify(options[operation][method], null, 2)
      : null;
  }
  ...
  defaultSecret(operation, method) {
    ...
    return secret[operation][method]
      ? JSON.stringify(secret[operation][method], null, 2)
      : null;
  }
